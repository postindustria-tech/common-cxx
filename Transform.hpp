 ...
 * Thus to be used in a multi-threaded setting - an instance per thread must be created.  This is also true for the SWIG-generated
 * wrappers of this class.
 *
 * Note the return values of the functions are copied value-types (map<string,string>) - thus memory in the internal buffer is safely
 * overwritten without harming the previously returned results - which is ideal for the (memory-)managed languages s.a. C#.
 */
...
namespace FiftyoneDegrees {
    namespace Common {
        class Transform {
            ...
            using CTransformAPI =
            fiftyoneDegreesTransformIterateResult (*)(const char* base64, char *buffer, size_t length,
        ...
        public:
            /**
             * Due to the default param value Transform() ctor is also available in C++ and managed language s.a. C#
             * @param capacity the size of the working memory buffer The buffer size will automatically double
             * if there is not enough memory to perform the conversion and the conversion will repeat with a bigger buffer.
             */
            Transform(size_t capacity = 1024);
            
            /**
             * Convert from getHighEntropyValues() results JSON string into HTTP header key-value pairs,
             * see help section to the `fiftyoneDegreesTransformGhevFromJson` routine defined in transform.h
            ...
            /**
             * Convert from the base64 encoded getHighEntropyValues() results JSON string into HTTP header key-value pairs,
             * see help section to the `fiftyoneDegreesTransformGhevFromBase64` routine defined in transform.h
             ...
             * see help section to the `fiftyoneDegreesTransformSUA` routine defined in transform.h
