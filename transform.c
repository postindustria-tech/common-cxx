...
// ------------------------------------------------------------------------------------------------
static size_t main_parsing_body(const char* json,
                                StringBuilder *builder,
                                Exception* const exception,
                                int isSua,
...
size_t TransformIterateGhevFromJsonPrivate(const char* json, StringBuilder *builder,
...
size_t fiftyoneDegreesTransformIterateGhevFromJson(
    ...
    size_t result = TransformIterateGhevFromJsonPrivate(json, builder, callback, ctx, exception);
    StringBuilderComplete(builder);
    return result;
}

size_t fiftyoneDegreesTransformIterateGhevFromBase64(
    const char* base64, StringBuilder *builder,
    fiftyoneDegreesTransformCallback callback, void* ctx,
    fiftyoneDegreesException* const exception) {
    StringBuilderInit(builder);
  size_t offset = base64_decode(base64, builder, exception);

  if (exception->status == CORRUPT_DATA || exception->status == INSUFFICIENT_MEMORY) {
    return 0;
  }
    char *json = builder->ptr;
    //note we are calling a private function to reuse the initialized stringbuilder
    size_t result = TransformIterateGhevFromJsonPrivate(builder->ptr, builder, callback, ctx, exception);
    StringBuilderComplete(builder);
    return result;
}
...
size_t fiftyoneDegreesTransformIterateSua(
    ...
    fiftyoneDegreesException* const exception) {
    StringBuilderInit(builder);
  size_t result = main_parsing_body(json, builder, exception, 1, callback, ctx);
    StringBuilderComplete(builder);
    return result;
}
...
size_t fiftyoneDegreesTransformGhevFromJson(
  ...
  size_t calls = TransformIterateGhevFromJson(
      json, builder, pushToHeaders, headers, exception);
...
size_t fiftyoneDegreesTransformGhevFromBase64(
  ...
  size_t calls = TransformIterateGhevFromBase64(
      base64, builder, pushToHeaders, headers, exception);
